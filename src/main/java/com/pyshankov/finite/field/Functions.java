...
public class Functions {
    ...
    public static double MDP(FiniteField field, Function<Polynom, Polynom> f) {
        double dfax = 0;
        for (int i = 1; i < field.getElements().size(); i++) {
            int[] b = new int[field.getElements().size()];

            long time = System.currentTimeMillis();

            for (int j = 1; j < field.getElements().size(); j++) {
                Polynom b1 = DaF(field.getElements().get(j),field.getElements().get(i), f);
                b[b1.toTenFromBinary()]++;
                if (b[j]>dfax) dfax = b[j];
            }
            System.out.println(System.currentTimeMillis()-time);
        }
        return dfax/Math.pow(2d, field.getModule().getDegree());
    }


    public static double MDP2(FiniteField field, List<Polynom> fFuncl) {
        double dfax = 0;
        for (int i = 1; i < field.getElements().size(); i++) {
            int[] b = new int[field.getElements().size()];
            ...
            for (int j = 1; j < field.getElements().size(); j++) {
                ...
                int x = fFuncl.get(j).getIndex();
                int a = fFuncl.get(i ^ j).getIndex();
